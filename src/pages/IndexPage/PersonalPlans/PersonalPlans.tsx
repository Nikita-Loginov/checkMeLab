...
const PersonalPlans: React.FC = () => {
  ...
  const personalPlans = [
    ...
    {
      name: 'Check.me Loy (Новинка)',
      description: 'Исследование лояльности. Типовой отчет по лояльности, вовлеченности, удовлетворенности и рисков увольнения на работе. Идеально для понимания себя и дополнительно для B2B услуг.',
      price: '2 000',
      originalPrice: '3 000',
      priceLabel: '',
      popular: false
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {personalPlans.map((plan, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className={`bg-white rounded-xl p-6 shadow-md hover:shadow-xl flex flex-col gap-3 transition-all duration-300 ${
                ...
                >
                  ...
                  <h4 className="text-lg font-bold text-primary-500 text-center">{plan.name}</h4>
                  <p className="text-xs text-gray-500 text-center">{plan.description}</p>
